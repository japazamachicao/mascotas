<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <button wire:click="goToPreviousWeek" class="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            </button>
            <button wire:click="goToCurrentWeek" class="px-3 py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 text-xs font-bold text-gray-700 transition">
                Esta Semana
            </button>
            <button wire:click="goToNextWeek" class="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 transition">
    ...
    <div class="text-center bg-indigo-50 border border-indigo-100 rounded-xl py-3 px-4 shadow-inner">
        <span class="text-sm font-black text-indigo-950 uppercase tracking-wide">
    ...
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-show="show" class="rounded-xl bg-green-50 p-4 border border-green-200 shadow-sm flex items-start justify-between">
        ...
        <div class="overflow-x-auto min-w-full">
            <div class="grid grid-cols-8 divide-x divide-gray-150 min-w-[900px] border-b border-gray-150">
                ...
                @foreach($daysOfWeek as $day)
                    <div class="p-3 text-center flex flex-col items-center justify-between {{ $day['is_today'] ? 'bg-primary-50/40' : 'bg-gray-50/20' }}">
                        <div class="flex flex-col items-center">
                            <span class="text-[10px] font-bold text-gray-400 uppercase leading-none">{{ $day['short_name'] }}</span>
                            <span class="text-lg font-black mt-1 leading-none {{ $day['is_today'] ? 'text-primary-600 bg-primary-100/60 rounded-full h-8 w-8 flex items-center justify-center' : 'text-gray-900' }}">
                                {{ $day['day_num'] }}
                            </span>
                        </div>
                        ...
                        <div class="mt-3 w-full">
                            @if($day['is_blocked'])
                                <button wire:click="unblockDate('{{ $day['formatted'] }}')"
                                        class="w-full py-1 px-2 text-[10px] font-bold bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded-lg transition flex items-center justify-center gap-1 shadow-sm">
                                    🔒 Desbloquear
                                </button>
                            @else
                                <button wire:click="openBlockModal('{{ $day['formatted'] }}')"
                                        class="w-full py-1 px-2 text-[10px] font-bold bg-gray-50 hover:bg-gray-100 text-gray-600 border border-gray-250 rounded-lg transition flex items-center justify-center gap-1">
                                    🔓 Bloquear Día
                                </button>
                            @endif
                        </div>
                    ...
                    <div class="grid grid-cols-8 divide-x divide-gray-150 align-stretch">
                        ...
                        @foreach($daysOfWeek as $day)
                            ...
                            <div class="p-1 min-h-[70px] relative transition-colors duration-200
                                ...
                                @if($isBlocked)
                                    <!-- Celda Bloqueada -->
                                    <div class="absolute inset-0 flex items-center justify-center bg-gray-150/20 backdrop-blur-[0.5px]" title="{{ $day['block_record']->notes ?? 'Día bloqueado manualmente' }}">
                                        <div class="text-center select-none">
                                            <span class="text-[10px] font-bold text-gray-400 flex items-center gap-1 bg-white border px-1.5 py-0.5 rounded shadow-sm">
                                                🔒 Bloqueado
                                            </span>
                                ...
                                @else
                                    ...
                                    @foreach($hourApps as $app)
                                        ...
                                        <a href="{{ route('dashboard.provider.appointments') }}" 
                                           class="block p-1.5 rounded-lg border text-[10px] leading-tight font-medium shadow-sm transition {{ $statusClass }} mb-1">
                                            ...
                                            <div class="text-[8.5px] text-gray-500 truncate mt-0.5">
                                                🐶 {{ $app->pet->name }}
                                            </div>
                                            <div class="text-[8.5px] text-gray-500 truncate">
                                                🛠️ {{ $app->services->pluck('name')->implode(', ') ?: 'Servicio base' }}
                                            </div>
                                        </a>
                                    @endforeach
                                @endif
    ...
    <!-- Leyenda de colores -->
    <div class="flex flex-wrap gap-4 items-center bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm text-xs font-semibold text-gray-600">
    ...
    @if($showBlockModal)
        <div class="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500/75 transition-opacity" aria-hidden="true" wire:click="closeBlockModal"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

                <div class="relative z-10 inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-md sm:w-full">
                    <div class="bg-white px-6 pt-6 pb-4 sm:pb-6">
                        <div class="sm:flex sm:items-start">
                            <div class="mx-auto shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-50 text-red-500 sm:mx-0 sm:h-10 sm:w-10">
                                🔒
                            </div>
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-base font-black text-gray-900" id="modal-title">
                                    Bloquear Fecha
                                </h3>
                                <div class="mt-2 text-xs text-gray-500">
                                    <p class="leading-relaxed">
                                        ¿Estás seguro de que deseas bloquear el día <strong>{{ \Carbon\Carbon::parse($selectedDate)->translatedFormat('d \d\e F, Y') }}</strong>? Los clientes no podrán agendar citas en este día.
                                    </p>
                                </div>
                                <div class="mt-4">
                                    <label class="block text-[10px] font-bold text-gray-700 uppercase tracking-wider mb-1">Notas / Motivo (Opcional)</label>
                                    <input type="text" wire:model="blockNotes" placeholder="Ej: Trámite médico, vacaciones, etc." 
                                           class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-primary-100 focus:border-primary-500 text-xs py-2.5 px-3 bg-gray-50/50">
                    ...
                    <div class="bg-gray-50 px-6 py-3 sm:flex sm:flex-row-reverse gap-2">
                        <button type="button" wire:click="blockDate" class="w-full sm:w-auto inline-flex justify-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg transition shadow-sm">
                            Bloquear
                        </button>
                        <button type="button" wire:click="closeBlockModal" class="mt-3 sm:mt-0 w-full sm:w-auto inline-flex justify-center px-4 py-2 border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 text-xs font-bold rounded-lg transition">
                            Cancelar
                        </button>
                    </div>
                </div>
